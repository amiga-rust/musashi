# MUSASHI [![Build Status][build-image]][build-status]

A portable Motorola M680x0 processor emulation engine.

## INTRODUCTION

_Musashi_ is a Motorola 68000, 68010, 68EC020, and 68020 emulator written in C.
...
It has been successfully running in the [MAME project][mame] for years
and so has had time to mature.

## LICENSE AND COPYRIGHT

_Musashi_ is available under the terms of the MIT license.
...
It is a good idea to take away the CPU's timeslice when it writes to a
memory-mapped port in order to give the device it wrote to a chance to
react.
...
- `M68K_EMULATE_PREFETCH` emulates the 4-word instruction prefetch that is part
  ...
  *NOTE*: if the CPU fetches a word or long word at an odd address when this
...
- `M68K_EMULATE_ADDRESS_ERROR` will cause the CPU to generate address error
  exceptions if it attempts to read a word or long word at an odd address.
...
[build-image]: https://travis-ci.org/amiga-rust/musashi.svg?branch=master
[build-status]: https://travis-ci.org/amiga-rust/musashi
